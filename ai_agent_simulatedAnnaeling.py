...
def simulated_annealing(game, max_time=4):
    start_time = time.time()
    end_time = start_time + max_time

    current_move = random.choice(game.get_valid_moves())
    current_score = evaluate_game_state(game, current_move)


    T = 1.0 
    while T > 0:
        T = scheduling_function(end_time = end_time, max_time = max_time) 
        if(T<=0):
            break
        new_move = random.choice(game.get_valid_moves())
        new_score = evaluate_game_state(game, new_move)
...
def evaluate_game_state(game, move):
    ...
    stability = calculate_stability(new_game)
