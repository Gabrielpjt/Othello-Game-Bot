...
def get_best_move(game, max_depth=8):
    """
    Given the current game state, this function returns the best move for the AI player using the Minimax algorithm
    with a specified maximum search depth.
    ...
    """
    _, best_move = minimax(game, max_depth)
    return best_move


def minimax(game, depth, maximizing_player=True):
    """
    Minimax algorithm for selecting the best move for the AI player.
